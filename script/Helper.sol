...
contract Helper {
    ...
    function getUsdcAddressFromNetwork(
        SupportedNetworks network
    ) internal pure returns (address usdcAddress) {
        if (network == SupportedNetworks.AVALANCHE_FUJI) {
            return usdcAvalancheFuji;
        } else if (network == SupportedNetworks.POLYGON_MUMBAI) {
            return usdcPolygonMumbai;
        } else if (network == SupportedNetworks.OPTIMISM_SEPOLIA) {
            return usdcOptimismSepolia;
        } else if (network == SupportedNetworks.BASE_SEPOLIA) {
